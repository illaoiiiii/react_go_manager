...
// 获取页面路径
export const getMenuPath = (list: Menu.MenuItem[] | undefined): string[] => {
	if (!list) {
		return []; // Return an empty array if list is undefined
	}
  return list.reduce((result: string[], item: Menu.MenuItem) => {
    //return result.concat(Array.isArray(item.children) && !item.buttons ? getMenuPath(item.children) : item.path + '')
		return result.concat(item.children?.[0]?.children?.length !== 0 ? getMenuPath(item.children) : item.path + '')
  }, [])
}
...
/**
 * 递归把children等于空数组变成null
 */
export function replaceEmptyChildrenWithNull(obj:any) {
	// 检查是否为对象
	if (typeof obj === 'object' && obj !== null) {
		...
		for (let key in obj) {
			if (obj.hasOwnProperty(key)) {
				// 递归调用函数来处理子属性
				obj[key] = replaceEmptyChildrenWithNull(obj[key]);
			}
		}
